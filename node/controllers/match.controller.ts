...
const initialize = async (req: Request, res: Response) => {
  try {
    ...
    res.send({
      status: 500,
      errorMessage: `Failed to initialize match due to: ${error}`,
    });
  }
};
...
const finalize = async (req: Request, res: Response) => {
  try {
    ...
    if (finalizedMatch) {
      ...
      res.send({
        status: 400,
        message: 'Match not found or is already complete.',
      });
    }
  } catch (error) {
    res.send({
      status: 500,
      errorMessage: `Failed to finalize match due to: ${error}`,
    });
  }
};

export { finalize, initialize };
